...
Rcpp::List empb_gamma_poisson_c_loop(NumericVector& ab,
                                     const double& G,
                                     const arma::colvec& Sx,
                                     const arma::colvec& mj,
                                     const double& eta,
                                     const double& tol,
                                     const int maxIter,
                                     int method) {
  ...
  arma::colvec B = b + mj;
  NumericVector ANV(G);
  ANV = as<NumericVector>(wrap(A));
  NumericVector BNV(G);
  BNV = as<NumericVector>(wrap(B));
  ...
  if(method == 0){
    ...
    while((abs(err) > tol) && (iternum < maxIter)){
      ...
      Score(0) = G * (log(b) - digamma(aNV)[0]) + sum(digamma(ANV) - log(BNV));
      Score(1) = G * a / b - sum(A / B);

      Hessian(0, 0) = sum(trigamma(ANV)) - G * trigamma(aNV)[0];
